...
def write_embedding_matrix(path: Path, labels: list[str], embeddings) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    ...
    with path.open("w", newline="", encoding="utf-8") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["id", "label", *feature_headers])
        for idx, embedding in enumerate(embeddings):
            writer.writerow([idx, labels[idx], *embedding])
...
def main() -> None:
    ...
    for language, config in LANGUAGE_CONFIG.items():
        ...
        if not args.no_embeddings:
            write_embedding_matrix(folder / config["embedding_matrix"], text_values, embeddings)
